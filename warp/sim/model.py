...
class Model:
    ...
    def state(self, requires_grad=None, lite=False) -> State:
        ...
        if self.composite_rigid_body_alg:
            ...
            s.joint_S_s = wp.zeros((self.joint_dof_count), dtype=wp.spatial_vectorf, requires_grad=requires_grad)

            # derived rigid body data (maximal coordinates)
            s.body_X_sc = wp.zeros((self.body_count), dtype=wp.transformf, requires_grad=requires_grad)
            s.body_X_sm = wp.zeros((self.body_count), dtype=wp.transformf, requires_grad=requires_grad)
            s.body_I_s = wp.zeros((self.body_count), dtype=wp.spatial_matrixf, requires_grad=requires_grad)
            s.body_v_s = wp.zeros((self.body_count), dtype=wp.spatial_vectorf, requires_grad=requires_grad)
            s.body_a_s = wp.zeros((self.body_count), dtype=wp.spatial_vectorf, requires_grad=requires_grad)
            s.body_f_s = wp.zeros((self.body_count), dtype=wp.spatial_vectorf, requires_grad=requires_grad)
            s.body_ft_s = wp.zeros((self.body_count), dtype=wp.spatial_vectorf, requires_grad=requires_grad)

            s.tmp = wp.zeros_like(self.joint_qd, requires_grad=requires_grad)

            # rigid contact data — 8 slots: 4 per foot x 2 feet
            s.point_vec = wp.zeros(self.articulation_count * 8, dtype=wp.vec3, requires_grad=requires_grad)
    ...
    def alloc_mass_matrix(self, requires_grad: bool = True):
        if (self.body_count):

            # system matrices
            self.M = wp.zeros(self.M_size, dtype=wp.float32, requires_grad=requires_grad)
            self.J = wp.zeros(self.J_size, dtype=wp.float32, requires_grad=requires_grad)
            self.P = wp.empty(self.J_size, dtype=wp.float32, requires_grad=requires_grad)
            self.H = wp.empty(self.H_size, dtype=wp.float32, requires_grad=requires_grad)
            self.L = wp.zeros(self.H_size, dtype=wp.float32, requires_grad=requires_grad)
            self.Jc = wp.zeros(self.Jc_size, dtype=wp.float32, requires_grad=requires_grad)
            self.G = wp.zeros(self.G_size, dtype=wp.float32, requires_grad=requires_grad)
            self.G_mat = wp.zeros((self.articulation_count, 8, 8), dtype=wp.mat33, requires_grad=requires_grad)
...
class ModelBuilder:
    ...
    def finalize(self, device=None, requires_grad=False) -> Model:
        ...
        with wp.ScopedDevice(device):
            ...
            # Per-articulation contact-smoothing scale (moreau soft prox). One
            # entry per articulation so it can be domain-randomized per env; the
            # soft kernels index it by tid. Filled uniformly (== legacy single
            # scalar) unless per-env contact-stiffness randomization is enabled.
            m.sigmoid_scale = wp.zeros(
                max(int(self.articulation_count), 1), dtype=wp.float32, requires_grad=requires_grad
            )
            m.col_height = 0.0
